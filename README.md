...
Both `quadtree_density` and `structure_energy` are built on **structure energy**: the gradient energy of the 4×-downsampled latent. Raw latent statistics can't measure softness — VAE latents carry a high-amplitude, high-frequency carrier regardless of pixel content. Downsampling averages the carrier away; what survives is actual image structure.

Structure energy's *absolute* magnitude isn't portable — it depends on which VAE encoded the latent and how much the canvas was upscaled before encoding (upscaling smooths real detail, lowering measured energy). Comparing every canvas against a fixed constant broke on every VAE/scale change. Both methods instead score a tile by where its energy ranks within *this canvas's own* distribution of tile energies — "where in this image is there more detail than elsewhere" is inherently relative, and ranking within the canvas is scale- and VAE-invariant by construction since it never looks at absolute magnitude. Because scores are always relative to their own canvas, they are **not comparable across different images or batches** — the same physical tile can score differently depending on what else is in its canvas, and a fixed `denoise_min`/`denoise_max`/`split_percentile` will not mean the same thing on two different images. A canvas with no meaningful energy spread anywhere (flat, or a uniform carrier) scores every tile 0 rather than promoting its least-flat tile to look detailed.

**`quadtree_density`** — Marks 4-latent-pixel blocks "hot" when their structure energy exceeds a value `split_percentile` of the way across the canvas's own p05–p95 block-energy range, then scores each tile by the fraction of its area covered by hot blocks. A value threshold rather than a count percentile: near-equal blocks (flat or carrier-only canvases) all sit below it instead of a fixed fraction being promoted to "hot". The scoring map shows the quadtree cell outlines built over the hot blocks (white on black) — large cells = flat, small cells = complex.

**`structure_energy`** — Scores each tile by where its structure energy ranks between the canvas's own 5th and 95th percentile of tile energies — the widest, most linear spread of the three methods. Because scores are canvas-relative, batch runs with fixed `denoise_min`/`denoise_max` treat each image independently — a mostly-soft image still gets its (relatively) busiest tiles denoised hardest. The scoring map is a grayscale image where white = at/above the canvas's own p95, black = at/below its p05.
...
| `split_percentile` | 0.6 | Optional, `quadtree_density` only. A cell subdivides while its structure energy exceeds this percentile of the whole canvas's own energy distribution. Lower = more of the image counts as detailed. Canvas-relative, so it holds steady across VAEs and upscale ratios. |
